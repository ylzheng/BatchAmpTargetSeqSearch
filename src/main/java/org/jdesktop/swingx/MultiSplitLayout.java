/*
 * $Id: MultiSplitLayout.java,v 1.1 2007-09-21 06:43:06 searle Exp $
 *
...
public class MultiSplitLayout implements LayoutManager {
	...
	if (root instanceof Leaf) {
	    ...
	    if (split.isRowLayout()) { 
                Iterator splitChildIter = splitChildren.iterator();
		while (splitChildIter.hasNext()) {
                    Node splitChild = (Node)splitChildIter.next();
	...
	if (split.isRowLayout()) {
	    ...
	    int totalWeightedWidth = 0;  // sum of the weighted childrens' widths
            Iterator splitChildIter = split.getChildren().iterator();
            while (splitChildIter.hasNext()) {
	        Node splitChild = (Node) splitChildIter.next();
   ...
   private void layout2(Node root, Rectangle bounds) {
	...
	if (root instanceof Divider) {
	    ...
	    Split split = (Split)root;
            Iterator childIter = split.getChildren().iterator();
	...
	if (nodeOverlapsRectangle(root, r) && (root instanceof Split)) {
	    List dividers = new ArrayList();
            Iterator childIter = ((Split)root).getChildren().iterator();
	    while (childIter.hasNext()) {
                Node child = (Node)childIter.next();
	...
	private Node siblingAtOffset(int offset) {
	    ...
	    index += offset;
	    return ((index > -1) && (index < siblings.size())) ? (Node)siblings.get(index) : null;
	}
	...
	public void setChildren(List children) {
	    ...
	    this.children = new ArrayList(children);
            childIter = this.children.iterator();
            while (childIter.hasNext()) {
                Node child = (Node)childIter.next();
	...
	try {
	    ...
	    parseSplit(st, root);
	    return (Node)root.getChildren().get(0);
	}
	catch (Exception e) {
