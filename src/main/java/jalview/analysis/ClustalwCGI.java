...
public class ClustalwCGI extends CGI {
  ...
  public ClustalwCGI(String server, int port,String location,AlignSequenceI[] sequence, PrintStream statout) {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      AlignSequenceI[] seqs = new AlignSequence[ff.getSeqs().size()];

      for (int i = 0; i < ff.getSeqs().size(); i++) {
        seqs[i] = (AlignSequence)ff.getSeqs().elementAt(i);
      }
