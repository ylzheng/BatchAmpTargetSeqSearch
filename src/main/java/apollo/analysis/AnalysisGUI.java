  ...
  private class NCBIBlastPane extends AnalysisPane
  {
    ...
    public String runAnalysis() throws Exception
    {
      RemoteBlastNCBI blast = new RemoteBlastNCBI((RemoteBlastNCBI.BlastType)runOptsPanel.getBlastType(),
          getOptions());
      CurationSet cs = CurationManager.getActiveCurationState().getCurationSet();
      // too much weirdness going on with the different implementation of Sequence objects, just create our own subsequence here
      int seqStart;
      int seqEnd;
      seqStart = start - cs.getLow();
      seqEnd = end - cs.getLow() + 1;
      SequenceI region = new Sequence(cs.getRefSequence().getName(), cs.getRefSequence().getResidues().substring(seqStart, seqEnd));
      //String type = blast.runAnalysis(cs, cs.getRefSequence().getSubSequence(start, end), strand, start - 1);      
      String type = blast.runAnalysis(cs, region, strand, start - 1);
