...
public class GFF3Adapter extends AbstractApolloAdapter
{
      ...
      SeqFeatureI parent, Map<String, Integer> writtenIds)
  {
    ...
    if (feat instanceof Transcript) {
      if (feat.isProteinCodingGene()) {
        ...
        int tEnd = trans.getLastBaseOfStopCodon(); //trans.getTranslationEnd();
  ...
  throws ApolloAdapterException
  {
    ...
    for (Map.Entry<String, GFF3Data> mapEntry : gffMap.entrySet()) {
      ...
      if (!added) {
        if (gff.isAnnotation()) {
          // need to wrap one-level annotations in a AnnotationFeatureI otherwise Apollo isn't happy
//          if (feat.isAnnotTop() && gff.isOneLevelAnnotation()) {
//            AnnotatedFeatureI fs = new AnnotatedFeature();
//            fs.addFeature(feat);
//            fs.setStrand(feat.getStrand());
//            fs.setId(feat.getId() + "-fs");
//            fs.setFeatureType(feat.getFeatureType());
//            feat = fs;
//          }
          // arbitrarily add strand-less features to plus strand
          if (feat.getStrand() == 0) {
    ...
    for (GFF3Data data : gffMap.values()) {
      ...
      if (gff.getType().equals("mRNA")) {
        ...
        if (coords != null) {
          ...
          int scds;
          int ecds;
          int fix; //used to fix stop codon Apollo uses the first base of
          //the stop codon rather than the last base as translation end


          //arrange CDS start and end by strand
          if (trans.getStrand() == -1) {
            scds = coords[1];
            ecds = coords[0];
            fix = 2;
          ...
          //only set start if CDS is real start_codon
          //or if CDS start and transcript start differ
          if(stran == scds){
            String cod = trans.na2aa(trans.getRefSequence().getResidues(stran, stran + (fix * -1)));
            //String cod = trans.getStartAA();
            if(cod.equals("M")){
          ...
          //let Apollo search for the end 
          //Check the last base againt the CDS end
          //If different use the CDS from the GFF3 file
          //This avoids forcing false stop codons
          //and instead lets translations just stop
          trans.setTranslationEndFromStart();
          int last = trans.getLastBaseOfStopCodon();
          if(last != ecds){
            trans.setTranslationEnd(ecds + fix); //fixed for first base of stop codon
          }
