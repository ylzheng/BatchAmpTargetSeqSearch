...
public class GFF3Entry
{
  ...
  private static Set<String> reservedAttributes;
  ...
  static {
    ...
    reservedAttributes = new TreeSet<String>(Arrays.asList(new String[] {
        "ID", "Name", "Dbxref", "Ontology_term", "Gap", "Alias", "Target", "Note", "Parent"
    }));
    tmpIdCounter = 0;
  }
  ...
  InstantiationException, IllegalAccessException
  {
    ...
    //non-GFF3 standard user defined attributes as feature properties
    for (Map.Entry<String, List<String> > entry : attrs.entrySet()) {
      if (isReservedAttribute(entry.getKey()) || entry.getKey().equals("Derives_from")) {
        continue;
      }
  ...
  private void parseData(SeqFeatureI feat)
  {
    ...
    src = feat.getProgramName() != null && feat.getProgramName().length() > 0 ?
        ...
        // phase only applies to CDS
        if (type.equals("CDS")) {
          setPhase(feat.getPhase());
        }
        id = feat.getId();
        name = feat.getName();
        //populate dbxrefs
        for (Object o : feat.getDbXrefs()) {
          ...
          //if dbxref is an ontology ID don't prepend the database
          if (dbxref.isOntology()) {
            key = "Ontology_term";
        ...
        //add other properties to attributes
        Hashtable hash = feat.getProperties();
        for (Object o : hash.entrySet()) {
          Map.Entry<String, String> e = (Map.Entry<String, String>)o;
          String key = e.getKey();
          String value = (String)e.getValue();

          //key can not equal reserved attributes
          //also ignore program which is redundant with the source tag
          if (isReservedAttribute(key) || key.equals("program")) {
            continue;
          }
  ...
  {
    return reservedAttributes.contains(key);
  }
}