...
public abstract class JdbcChadoAdapter {
      ...
      boolean setAlignSeqs, FeatureLocImplementation featLoc)
  {
    ...
    */
    ...
    try {
      ...
      (getChadoInstance().searchHitsHaveFeatLocs() ? ", leafToParent.object_id AS parent_id" : "") +
      ...
      + ", tgtFeat.uniquename "
      + ", leafFeatLoc.strand "
      + (getChadoInstance().searchHitsHaveFeatLocs() ? ", leafToParent.object_id" : "");
      ...
      while (rs.next()) {
        ...
        long parentId = getChadoInstance().searchHitsHaveFeatLocs() ? rs.getLong("parent_id") : 0;
        ...
        if (alignType.equals("SNP")) {
          ...
          featName = "SNP (" + queryResidueInfo + "src" + tgtResidueInfo + ")";
  ...
  private void setTranscriptNameAndCdsBounds(FeatureSet tfs, String transName, String program, Hashtable cdsHash) {
    ...
    if (logger.isDebugEnabled()) {
      ...
      msg.append(tfs.getName() + ":" + tfs.getStart() + "src" + tfs.getEnd() +
      ...
      for (Enumeration e = ve.elements(); e.hasMoreElements();) {
        ...
        logger.debug("exon: " + sf.getStart() + "src" + sf.getEnd());
      }
    }
  }
  ...
  void addOneLevelResults() {
    ...
    try {
      String chadoProgramWhereClause = getChadoProgramWhereClause(programs, "af", analysisAlias);
      // no analysis found for configured programs
      if (chadoProgramWhereClause.length() == 0) {
        return;
      }
      where.add(chadoProgramWhereClause);
    }
    catch (AnalysisException e) {
