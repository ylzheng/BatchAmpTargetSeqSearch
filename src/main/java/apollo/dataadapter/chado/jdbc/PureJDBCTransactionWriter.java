...
public class PureJDBCTransactionWriter {
  ...
  */
  ...
  protected Long addSequenceEdit(SequenceEdit seqEdit, String author, Date date) {
    ...
    ChadoFeature  srcFeature = new ChadoFeature(getSeqEditRefFeature(seqEdit), this.jdbcAdapter.getConnectionUsedForLastTransaction());
    long organismId = srcFeature.getOrganismId();
    long refseqFeatureId = srcFeature.getFeatureId();
    ...
    if (name == null) {
      name = String.format("%s-%s-%d-%s", seqEdit.getEditType(), seqEdit.getRefSequence().getName(), seqEdit.getPosition(), seqEdit.getResidue());
    }
  ...
  protected AnnotatedFeatureI getSeqEditRefFeature(SequenceEdit seqEdit) {
    AnnotatedFeatureI refFeature = (AnnotatedFeatureI)seqEdit.getRefFeature("exon");
    if (refFeature != null) {
      return refFeature;
    }
    return seqEdit;
