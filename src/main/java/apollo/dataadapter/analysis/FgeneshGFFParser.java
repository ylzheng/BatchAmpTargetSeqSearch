...
public class FgeneshGFFParser extends GeneFinderParser {

  public FgeneshGFFParser () {
  }
	...
	if (!isCommentary()) {
	  ...
	  if (parsed) {
		...
		gene_id = tokens.nextToken();
	      trans_id = (tokens.hasMoreTokens()) ? tokens.nextToken() : "";
	      if (trans_id.startsWith("trans") && tokens.hasMoreTokens())
		trans_id = tokens.nextToken();
	    }
	  }
	}
	// debugLine (line);
