...
public class BaseEditorPanel extends SeqAlignPanel {
  ...
  /** 
   * Adds all results in a set to this panel so
   * that they can be displayed.
   * 
   * @param Set<SeqFeatureI> reults - each result should be <code>!isAnnot</code>
   * @return <code>int</code> tier - the number of results added
   *  
   */
  public int addResults(Set<SeqFeatureI> results) {
    int count = 0;
  ...
  private Vector getTranscriptsAnd1LevAnnots(SeqFeatureI annotFeatureSet) {
    ...
    else if (annotFeatureSet.isAnnotTop() && !annotFeatureSet.hasKids()) {
      output.add(annotFeatureSet);
    }
    else {
    ...
    public void doSearch() {
      ...
      if (isAminoAcidButton.isSelected()) {
        for(int j=0; j < 3; j++) {
          ...
          setTranslatedHitZones(matches, j); //Whats the difference between hitzone and translated hit zone?
  ...
  public int attachAnnot(AnnotatedFeatureI annot) {
    ...
    if (trans_tier >= current_tier_count) {
      ...
      attachRendererToTier(trans_tier, renderer);
    }
    return trans_tier;
  }
  
  /** 
   * Adds a result to this panel so that it can
   * be displayed.
   * 
   * @param SeqFeatureI result - should be <code>!isAnnot</code>
   * @return <code>int</code> tier - the tier on which the result was placed
   *  
   */
  public int addResult(SeqFeatureI result) {
    int current_tier_count = getTierCount();
    int trans_tier = addFeature(result);
    if (trans_tier >= current_tier_count) {
        ResultRenderer renderer =
          new ResultRenderer (this,
                              getCharWidth(),
                              getCharHeight());
        renderer.setFont(getFont());
        attachRendererToTier(trans_tier, renderer);
  ...
  public void attachTranscripts(Vector transcripts) {
    ...
    // reverse complement the refSeq if strand is reverse
    if (reverseStrand) {
      SequenceI temp_seq = refSeq;
      RangeI temp_range = temp_seq.getRange();
      
      refSeq = new Sequence(temp_seq.getName(),
                            temp_seq.getReverseComplement());
      refSeq.setResidueType(temp_seq.getResidueType());
      
      refSeq.setRange(new Range(temp_range.getName(), //the range wont be the curation set...
                                temp_range.getEnd(),  //does that matter?
                                temp_range.getStart()));
    }
    ...
    AminoAcidRendererOLD aar = new AminoAcidRendererOLD(char_width,
        char_height,
        0,
        this);
    ...
    addTier (frame[0], 0, SequenceI.AA, 0);
    attachRendererToTier(0,aar);

    aar = new AminoAcidRendererOLD(char_width,
        char_height,
        1,
        this);
    aar.setFontSizes(getFont().getSize());
    ...
    addTier (frame[1], 1, SequenceI.AA, 1);
    attachRendererToTier(1,aar);
    ...
    addTier (frame[2], 2, SequenceI.AA, 2);
    attachRendererToTier(2,aar);
