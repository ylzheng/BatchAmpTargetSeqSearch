...
public class BrowserLauncher {
  ...
  static {
    ...
    if (osName.startsWith("Mac OS")) {
      ...
      String majorMRJVersion = mrjVersion.substring(0, (mrjVersion.length() >= 4) ? 3 : mrjVersion.length()-1);
      try {
        ...
        if (version == 2) {
          ...
          // Does 4 behave the same as 3?
          // Note that with 4.1 (I think),
          // System.getProperty("mrj.version"); is returning 99!!
          // But it seems to work ok to treat that as MRJ_3_1.
          jvm = MRJ_3_1;
        } else {
          loadedWithoutErrors = false;
          errorMessage = "Unsupported MRJ version: " + version;
          //          System.out.println("BrowserLauncher: " + errorMessage); // DEL
        }
        //                                System.out.println("BrowserLauncher: mrjVersion = " + mrjVersion + ", version num = " + version + ", jvm = " + jvm); // DEL
      ...
      //      System.out.println("BrowserLauncher: osName = " + osName + ", java.home = " + System.getProperty("java.home") + ", java.version = " + System.getProperty("java.version")); // DEL
      jvm = OTHER;
    }
  ...
  private static boolean loadClasses() {
    ...
    case MRJ_3_1:
      try {
        ...
        errorMessage = cnfe.getMessage();
        //        System.err.println("BrowserLauncher.init: error: " + errorMessage); // DEL
        return false;
      } catch (NoSuchMethodException nsme) {
        errorMessage = nsme.getMessage();
        //        System.err.println("BrowserLauncher.init: error: " + errorMessage); // DEL
  ...
  /** 
   * If browser is known can set it. I know this goes against the grain of this class but this 
   * class only tries using "netscape" on Linux, which doesn't necasarily work and I dont know how 
   * to get it to look for a browser on linux(that would be the ideal). So this just overrides
   * the automatic loadBrowser method. (MG)
   */
  ...
  public static void openURL(String url) throws IOException {
    ...
    case WINDOWS_9x:
      ...
      break;
    case OTHER:        // Assume that we're on Unix
      // First try using the defined browser string to redirect the open browser
      // to the requested URL.
      if (!redirectOpenBrowser((String)browser,
                               NETSCAPE_REMOTE_PARAMETER,
                               NETSCAPE_OPEN_PARAMETER_START,
                               url,
                               NETSCAPE_OPEN_PARAMETER_END)) {
        // 2/2004: recent versions of mozilla are set up to run with a shell
        // script (run-mozilla.sh) which doesn't handle redirect commands.
        // So if that's what our browser was set to, we should try finding
        // the actual mozilla binary and launching a redirect command to it.
        // If that doesn't work, then we probably don't have an open mozilla,
        // and we should go back to the shell script and run it first to
        // launch a new mozilla, and then use the binary to redirect.
        // (YUCK!)
        String redirectableBrowser = findBinary((String)browser);
        if (redirectOpenBrowser(redirectableBrowser,
                                NETSCAPE_REMOTE_PARAMETER,
                                NETSCAPE_OPEN_PARAMETER_START,
                                url,
                                NETSCAPE_OPEN_PARAMETER_END)) {
          browser = redirectableBrowser;
          apollo.config.Config.setBrowserProgram(redirectableBrowser); // aaaaah!
        }
        else {
          // redirectableBrowser didn't work either--probably we don't have 
          // mozilla running yet.
          // First launch a new browser using the original shell script.
          try {
            System.out.println("Launching new web browser: " + browser + " " + url);
            Runtime.getRuntime().exec(new String[] { (String) browser });
            // Now tell the running mozilla to go to a particular URL
            // but use the mozilla xremote client (if it exists), because
            // the newer run-mozilla.sh doesn't pass along arguments correctly.
            // First sleep a bit to give the browser a chance to launch.
            // YUCK!
            Thread.currentThread().sleep(8000);
          }
          catch (Exception e) {
            System.err.println("Exception while launching browser: " + e.getMessage());
            return;
          }
          // Now use the redirectable browser
          browser = redirectableBrowser;
          apollo.config.Config.setBrowserProgram(redirectableBrowser); // aaaaah!
          if (!redirectOpenBrowser((String)browser,
                                   NETSCAPE_REMOTE_PARAMETER,
                                   ...
                                   url,
                                   NETSCAPE_OPEN_PARAMETER_END)) {
            System.out.println("Could not point open browser " + browser + " to requested url " + url);
          }
        }
      }
      break;
  ...
  /** Attempt to open the URL in currently running browser */
  private static boolean redirectOpenBrowser(String browser, String remote, String open, String url, String close) {
    try {
      ...
      if (exitCode == 0)
        ...
        //        System.out.println("redirectOpenBrowser failed"); // DEL
        return false;
      }
    } catch (Exception ie) {
      System.err.println("Exception while launching browser: " + ie.getMessage());
      return false;
    }
  }
  
  /** Given a browser pathname representing a shell script (e.g. /opt/mozilla-1.6/run-mozilla.sh),
   * return the name of the binary that should correspond to that shell script
   * (e.g. mozilla) */
  private static String findBinary(String br) {
    String orig_browser = br;
    if ((br.indexOf("mozilla") >= 0) && (br.indexOf(".sh") > 0) && (br.indexOf("/") >= 0)) {
      br = br.substring(0, br.lastIndexOf("/"));
      br = br + "/mozilla";
      File handle = new File(br);
      if (handle.exists()) {
        return br;
      }
    }
    System.out.println("Couldn't find binary corresponding to browser script " + orig_browser);
    return orig_browser;
  }
